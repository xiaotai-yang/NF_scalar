...
@partial(jax.jit, static_argnames=('num_samples','L','t0', 'tf', 'dt'))
def loss_func_path(params, seed, num_samples, diff_xf_W_t0, diff_xf_omega_t0, eta, L, t0, tf, dt):
    W_a0, omega_f = params
    print("W_a0:", W_a0.shape)
    W_a = jnp.transpose(vmap(vmap(initial_mask, (0,)), (1,))(W_a0), (1, 0, 2, 3))
    print("W_a:", W_a.shape)
    x0, logp_x0, dlogp_x0 = get_batch(num_samples, L, seed)
    xf, logp_prob, diff_logp_x, int_diff_xf_W, int_diff_xf_omega, kine_W, kine_omega = rk4_odeint_path(
        (x0, jnp.zeros(x0.shape[0]), dlogp_x0, diff_xf_W_t0, diff_xf_omega_t0, jnp.zeros_like(W_a0), jnp.zeros_like(omega_f))
    ...
    grad_w = jnp.mean(jnp.fft.ifft2(
        jnp.flip(jnp.roll(jnp.fft.fft2(int_diff_xf_W), (-1, -1), axis=(-2, -1)), (-2, -1)) * jnp.fft.fft2(
            diff_logp_x)[:, None, None]).real + eta*kine_W, axis=0)
    grad_omega = jnp.mean(jnp.sum(diff_logp_x[:, None] * int_diff_xf_omega, axis=(-2, -1)) + eta*kine_omega, axis=0)
...
parser.add_argument("--L", type=int, default=4, help="Size of the system")
...
parser.add_argument("--eta", type=float, default=1e-3, help="Kinetic regularization parameter")
...
for i in range(train_steps):
    t0_ = time.time()
    if method == "path":
        loss, ess, grad_wa0, grad_omega = loss_func_path(params, seed, num_samples, diff_xf_W_t0, diff_xf_omega_t0, eta, L, t0, tf, dt)
